...
if(isset($_GET['id'])){
    ...
    $query = $db->query("SELECT * FROM clientes WHERE id = ".$_GET['id']);
    if($query->rowCount() == 0){echo "Nenhum dado encontrado com este id..."; } else {
    $customers = $query->fetchAll();
        if(isset($_POST['id'])){
            atualizar("clientes","`nome`='".$_POST['nome']."',`sobrenome`='".$_POST['sobrenome']."',`email`='".$_POST['email']."',`telefone`='".$_POST['telefone']."',
